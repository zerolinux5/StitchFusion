...
def main(cfg, gpu, save_dir):
    ...
    loss_cfg, optim_cfg, sched_cfg = cfg['LOSS'], cfg['OPTIMIZER'], cfg[
        'SCHEDULER']
    ...
    trainset = eval(dataset_cfg['NAME'])(dataset_cfg['ROOT'], 'train',
                                         traintransform, dataset_cfg['MODALS'])
    valset = eval(dataset_cfg['NAME'])(dataset_cfg['ROOT'], 'val',
                                       valtransform, dataset_cfg['MODALS'])
    ...
    for epoch in range(start_epoch, epochs):
        ...
        pbar = tqdm(
            ...
            desc=
            f"Epoch: [{epoch+1}/{epochs}] Iter: [{0}/{iters_per_epoch}] LR: {lr:.8f} Loss: {train_loss:.8f}"
        )
        metrics = Metrics(trainset.n_classes, trainloader.dataset.ignore_label,
                          device)
            ...
            if (train_cfg['DDP'] and torch.distributed.get_rank()
                ...
                if miou > best_mIoU:
                    ...
                    torch.save(
                        ...
                        if train_cfg['DDP'] else model.state_dict(), cur_best)
                    # ---
                    torch.save(
                        {
                            'epoch':
                            ...
                            'model_state_dict':
                            model.module.state_dict()
                            if train_cfg['DDP'] else model.state_dict(),
                    ...
                    logger.info(
                        print_iou(epoch, ious, miou, acc, macc, class_names))
                logger.info(
    ...
    table = [['Best mIoU', f"{best_mIoU:.2f}"],
             ['Total Training Time',
